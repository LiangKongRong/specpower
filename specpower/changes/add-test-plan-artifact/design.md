...
- **Step2 — AST semantic (deep, staged, best-effort)**: locate that Case's test by prefix token, AST-parse, and **best-effort** verify that assertions match the plan's input/expected. **R2 softening**: NL input/expected ↔ code assertions is itself a semantic match (e.g. does `expect(x).toThrow('name required')` count as matching the expected "throw EINVAL 'name required'"?) — not fully automatable; Step2 reports a **warning with confidence/gap** (not a hard pass/fail). **Staged**: v1 ships Step1 (reliable) + Step2 best-effort warn; v2 deepens Step2 per AST framework.
...
- **Step2 minimal-checkable items**: Step2 verifies "the prefix token's `it()` exists + AST is parseable + the function under test is called + the Case's input is touched"; **it does not judge semantic equivalence** (e.g. it does not judge "the assertion's expected wording equals the plan's expected"). Unable to satisfy the minimal items → warn (not fail). This is the explicit boundary after R2's softening.
...
- **Q3 — v2 Step2 AST frameworks**: which frameworks does v2 support (vitest/jest first; python/go later). v1 ships Step1 (`[Tn]` token scan) + Step2 best-effort warn. (The `[Tn]`-name id-embedding convention and the id-stability rule are already set in R2.)
