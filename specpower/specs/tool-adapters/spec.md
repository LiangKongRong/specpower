...
- **`skill-tool` tools (claude/cac/chrys)** — these runtimes expose a host Skill tool that resolves skills by name. The body SHALL name the skill by its canonical directory name (`specpower-<cmd>`) and direct the model to load it by calling the **Skill tool**. This form is path-free and scope-agnostic, so it works unchanged for both project and user scope and avoids `~` path expansion (the Skill tool discovers both project and user skills by name).
...
The body MAY remain terse (it is a dispatch stub, not the procedure itself — the SKILL.md remains the single source of the staged procedure), but it MUST carry the mechanism-appropriate loading instruction plus the three guarantees above so the dispatch is reliable across all tools regardless of whether arguments are present.
...
- **AND** the body contains no `Read `<path>`` instruction (it is path-free and scope-agnostic)
...
- **THEN** the body contains `Read \`~/.opencode/agent/specpower-<cmd>.md\`` (user-scope skills live under the home dir)
